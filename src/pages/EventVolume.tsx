    ...
    <>
      ...
      {summary.isLoading ? <LoadingSpinner /> : summary.error ? <ErrorAlert error={summary.error} /> : events ? (
        <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
          ...
          <MetricCard title="Matched Rate" value={formatPercentage((events.processingStatusBreakdown.matched / events.totalEvents) * 100)} />
          <MetricCard title="Zero Match" value={formatPercentage((events.processingStatusBreakdown.zeroMatch / events.totalEvents) * 100)} />
          <MetricCard title="Duplicate Rate" value={formatPercentage((events.processingStatusBreakdown.duplicate / events.totalEvents) * 100)} />
        </div>
      ) : null}
